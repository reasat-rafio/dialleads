...
<div class="flex h-full w-full flex-col rounded-[16px] border border-gray-200 p-[30px]">
  ...
  <div class="mt-9 h-full w-full space-y-2.5">
    ...
    <p class="w-full font-geist text-[16px] font-normal leading-[150%] tracking-[0%] text-[#5B6779]/80">
  ...
  <a
    href={card?.ctaBtnURL ?? "/"}
    class={`group relative mt-[30px] flex h-[52px] w-full items-center justify-center gap-3 overflow-clip rounded-[0.75rem] border border-violet-600 bg-transparent px-7 py-4 text-violet-600 transition-all duration-200 ease-linear after:absolute after:inset-y-0 after:right-[100%] after:z-[-1] after:w-[150%] after:transition-all after:duration-300 after:ease-linear after:content-[''] after:[background-image:_linear-gradient(to_right,_rgba(113,48,214,1.0)_0%,_rgba(177,136,240,1.0)_70%,_rgba(177,136,240,0.0)_100%)] hover:bg-transparent hover:text-white hover:after:right-[-40%]`}>
    <SanityImage
      ...
      alt={"icon"} />
    <span class="w-fit max-w-full text-nowrap font-geist text-[18px] font-normal leading-[18px] tracking-normal">
      {card.ctaBtnText}
    </span>
  </a>
</div>
