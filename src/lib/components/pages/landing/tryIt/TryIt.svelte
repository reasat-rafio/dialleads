<script lang="ts">
	...
	$effect(() => {
		...
		if (dialCode && !$formData.phoneNumber.startsWith(dialCode)) {
			...
			$formData.phoneNumber = dialCode + $formData.phoneNumber.replace(/^\+\d*/, '');
		}
	});
</script>
...
<div class="relative mb-20 mt-[73px] px-5">
	...
	>
		...
		<div class="relative z-10 h-fit w-full px-5 pb-6 pt-[52px] lg:px-[50px] lg:py-[40px]">
			...
			<form class="mt-6 flex" method="POST" use:enhance>
				<div class="grid w-full grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
								...
								{#snippet children({ props })}
									<div class="relative w-full">
										...
										<div class="absolute bottom-5 left-4">
											<svg
												class="size-[1rem] md:size-[1.25rem]"
												viewBox="0 0 25 25"
												fill="none"
					...
					<div class="w-full">
						<div
							class="relative flex h-full min-h-[3.5rem] w-full items-center gap-x-2 rounded-[0.75rem] bg-white px-4 md:min-h-[3.75rem]"
						>
							<div class="flex h-full w-fit items-center gap-x-2">
								<svg
									class="size-[1rem] md:size-[1.25rem]"
									width="25"
									height="25"
								...
								<Select.Root type="single" name="country" bind:value={selectedCountry}>
									<Select.Trigger class="w-fit text-lg focus:ring-0">
										{#if selectedCountry}
							...
							<Form.Field form={testCallForm} name="phoneNumber" class="w-full">
								<Form.Control>
									{#snippet children({ props })}
										<div class="relative w-full">
											<Input
					...
					>
						<SanityImage
							class="h-5 w-5"
							src={props?.ctaBtnIcon}
							sizes="20px"
							imageUrlBuilder={imgBuilder}
							alt={'icon'}
						/>
						<span>{props.ctaBtnText}</span>
					</Form.Button>
				</div>
			</form>
		</div>
