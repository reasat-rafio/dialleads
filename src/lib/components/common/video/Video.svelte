    ...
    {#if !isPlaying}
      <button onclick={playVideo} class="group relative block h-full w-full" aria-label="Play video">
        ...
        <div class="absolute inset-0 flex items-center justify-center">
          <div
            class="flex size-10 items-center justify-center rounded-full border border-violet-500 bg-white/50 shadow-[0px_16px_35.3px_0px_rgba(76,29,149,0.25)] transition-transform group-hover:scale-110 sm:size-16 lg:size-20">
            <Play class="ml-1 h-6 w-6 fill-violet-600 text-violet-600 sm:h-7 sm:w-7 lg:h-8 lg:w-8" />
          </div>
        </div>
      </button>
    {:else}
