<script lang="ts">
  ...
  import type { ImageUrlBuilderOptionsWithAliases, SanityImageSource } from "@sanity/image-url/lib/types/types";
  import type { HTMLImgAttributes } from "svelte/elements";
  import { defaultSanityImageDefaults } from "./defaults";
  import { generateWidths, isSanityDimensionedImage, isSanityImageWithLqip } from "./fns";
  ...
  let {
    ...
    innerClass,
    ...rest
  }: Props & SanityImageProps & Omit<HTMLImgAttributes, "src"> & { fadeInAnimation?: boolean } = $props();

  function imgProps() {
    const builder = imageUrlBuilder?.image(src).withOptions({ auto: autoFormat ? "format" : undefined });
    ...
    const lqipStyle = `background: ${
      isSanityImageWithLqip(src) ? `url(${src.asset.metadata.lqip}) no-repeat` : undefined
